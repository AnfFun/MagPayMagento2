...
class TransactionBuilder
{
    ...
    public function captureOrderTransaction(
        ...
        // Only create invoice if not Credit Card with Manual Capture enabled
        if (!($method == 'ginger_methods_creditcard' && $isManualCaptureEnabled)) {
        ...
        if ($order->getIsVirtual()) {
            ...
            if ($isManualCaptureEnabled) {
                // If manual capture is enabled, keep the current state (authorized)
                $status = $this->configRepository->getStatusAuthorized($method, (int)$order->getStoreId());
                $order->addStatusToHistory($status, __('Payment has been authorized.'), false);
            } else {
    ...
    public function processRequest(OrderInterface $order, $transaction = null, $testModus = null): array
    {
        ...
        if ($transactionId && !$this->configRepository->getError($transaction)) {
            ...
            // Use authorized status if manual capture is enabled, otherwise use pending status
            if ($this->configRepository->isManualCaptureEnabled($method, (int)$order->getStoreId())) {
                $status = $this->configRepository->getStatusAuthorized($method, (int)$order->getStoreId());
            } else {
    ...
    public function complete(array $transaction, OrderInterface $order, string $type): array
    {
        ...
        if (!$payment->getIsTransactionClosed() && $type == 'webhook') {
            ...
            if ($this->configRepository->isManualCaptureEnabled($method, (int)$order->getStoreId())) {
                // If manual capture is enabled, use authorized status
                $status = $this->configRepository->getStatusAuthorized($method, (int)$order->getStoreId());
            } else {
                // Otherwise use processing status

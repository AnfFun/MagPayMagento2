...
class PaymentLibrary extends AbstractMethod
{
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        if ($this->configRepository->isManualCaptureEnabled((string)$this->_code, (int)$order->getStoreId())) {
            $status = $this->configRepository->getStatusAuthorized((string)$this->_code, (int)$order->getStoreId());
        } else {
    ...
    public function prepareTransaction(OrderInterface $order, $platformCode, $methodCode): array
    {
        ...
        $paymentDetails = $this->orderDataCollector->getTransactions($platformCode, $verifiedTermsOfService, $methodCode);
